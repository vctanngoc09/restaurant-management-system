    ...
    public User registerUser(SignupRequest signUpRequest) {
        ...
        if (strRoles == null || strRoles.isEmpty()) {
            Role waiterRole = roleRepository.findByName(ERole.ROLE_WAITER)
                    .orElseThrow(() -> new RuntimeException("Lỗi: Không tìm thấy quyền WAITER trong DB."));
                ...
                switch (role.toLowerCase()) {
                    ...
                    case "chef":
                        Role chefRole = roleRepository.findByName(ERole.ROLE_CHEF)
                                .orElseThrow(() -> new RuntimeException("Lỗi: Không tìm thấy quyền CHEF."));
                        roles.add(chefRole);
                        break;
                    case "cashier":
                        Role cashierRole = roleRepository.findByName(ERole.ROLE_CASHIER)
                                .orElseThrow(() -> new RuntimeException("Lỗi: Không tìm thấy quyền CASHIER."));
                        roles.add(cashierRole);
                        break;
                    case "waiter":
                    default:
                        Role waiterRole = roleRepository.findByName(ERole.ROLE_WAITER)
                                .orElseThrow(() -> new RuntimeException("Lỗi: Không tìm thấy quyền WAITER."));
                        roles.add(waiterRole);
                }
            });
        }
